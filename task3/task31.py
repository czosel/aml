...
def process(X):
    ...
    for i in range(len(X)):
        ...
        rdiffs = np.log(rpeaks_s[1:] - rpeaks_s[:-1])
        if len(rdiffs) > 150:
            d = int((len(rdiffs) - 150)/2)
            rdiffs = rdiffs[d:d+150]
        # plt.plot(rdiffs)
        # plt.show()
        features.append(rdiffs)
...
process(X).to_csv("features/LSTMCache/train_X.csv", index=False)
np.savetxt("features/LSTMCache/train_y.csv", y, delimiter=",")
process(X_test).to_csv("features/LSTMCache/test_X.csv", index=False)
